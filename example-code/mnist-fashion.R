...
input.data.set <- DataSetReader(1, 60000, "fashiontraininput.csv", normalise = TRUE)
expected.output.data.set <- DataSetReader(1, 60000, "fashiontrainoutput.csv")
input.testing <- DataSetReader(1, 10000, "fashiontestinput.csv", normalise = TRUE)
expected.output.testing <- DataSetReader(1, 10000, "fashiontestoutput.csv")
...
dictionary <- c("T-shirt/top", "Trouser", "Pullover", "Dress", "Coat", "Sandal", "Shirt", "Sneaker", "Bag", "Ankle Boot") # dictionary of items

cat("The image is a ", dictionary[which(results[,image.number] == max(results[,image.number]))], "\n", sep = "")
